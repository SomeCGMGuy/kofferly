...
const dialogs = [...document.querySelectorAll("dialog.modal:not(#notificationDialog):not(.sheet-dialog):not(.alert-dialog)")];
...
function closeDialog(dialog, returnValue = "") {
  ...
  if (reduceMotion.matches) return finishClose(dialog, returnValue);
...
for (const dialog of dialogs) {
  ...
  dialog.showModal = () => { nativeShowModal(); enterDialog(dialog); };
...
document.addEventListener("click", event => {
  ...
  if (!dialog || !form || !dialog.classList.contains("push-dialog")) return;
...
for (const dialog of dialogs) {
  ...
  dialog.addEventListener("pointerup", event => {
    if (event.target === dialog && dialog.dataset.backdropPointer === "true") closeDialog(dialog, "cancel");
